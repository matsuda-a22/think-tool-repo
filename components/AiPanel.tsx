...
function extractResponseText(response: GenerateContentResponse): string {
  ...
  if (fromGetter != null && fromGetter.trim()) return fromGetter.trim()
...
function describeEmptyGeminiResponse(response: GenerateContentResponse): string {
  ...
  if (!response.candidates?.length) {
    return "APIから応答がありませんでした。APIキーとネットワーク接続を確認してください。"
  }
  if (reason === "MAX_TOKENS") {
    return "出力トークン上限に達し、本文が生成されませんでした（思考モードで枠を使い切った可能性があります）。もう一度「提案を取得」を押してください。"
  }
  if (reason === "SAFETY") {
    return "安全フィルターにより回答がブロックされました。書き出し内容を変えて再試行してください。"
  }
  if (reason === "RECITATION") {
    return "著作権フィルターにより回答がブロックされました。"
  }
  const msg = candidate?.finishMessage?.trim()
  if (msg) return `回答を取得できませんでした: ${msg}`

  return "モデルからテキストが返りませんでした。Web検索をOFFにするか、しばらく待って再試行してください。"
}
...
async function callGemini(
  ...
  const response = await ai.models.generateContent({
    ...
    config: {
      ...
      // 2.5 Flash はデフォルトで思考にトークンを使い、本文が空になることがある
  ...
  if (suggestions.length === 0) {
    throw new Error(
      "回答は受け取れましたが、提案の形式を読み取れませんでした。もう一度「提案を取得」を押すか、書き出し内容を少し増やしてください。",
    )
  }
...
export default function AiPanel({
  ...
  useEffect(() => {
    if (initialScope) {
      setContextScope(initialScope)
      if (initialScope.kind === "memo" || initialScope.kind === "fine") {
        setInsertedFineThemeId(initialScope.fineId)
      }
    }
  }, [initialScope])
  ...
  return (
    <div className="border-b bg-muted/30 flex flex-col h-full min-h-0 overflow-hidden">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        <div className="px-3 py-2 space-y-1.5 min-h-[100px]">
          {error && (
          ...
          {activeResult && (
            <>
              {activeResult.suggestions.length === 0 && (
                <p className="text-xs text-amber-800 dark:text-amber-200 rounded-md bg-amber-500/10 border border-amber-500/30 px-2 py-2">
                  提案が生成されませんでした。もう一度「提案を取得」を押してください。
                </p>
              )}
